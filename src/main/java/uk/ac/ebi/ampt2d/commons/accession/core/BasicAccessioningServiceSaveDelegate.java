...
/**
 * This class is delegate for {@link BasicAccessioningService} to manage the insert operation.
 * <p>
 * The insert operation can fail when an object has been already accessioned with a different value while having the
 * same hash message and elements. In this case a database constraint exception can be raised. Due to limited
 * information in those cases of error on JDBC driver, the insert operation is implemented as an iterative version of a
 * binary partition.
 * <p>
 * If the insert operation works, all elements have been stored, otherwise, because the insert operation is expected to
 * be transactional, the saved elements are dropped from database. And we split in two the batch of elements to insert
 * and try to insert each part separately until all the split batches have been saved correctly and we find the specific
...
class BasicAccessioningServiceSaveDelegate<MODEL, HASH, ACCESSION> {
    ...
    /**
     * Perform insert operation, return saved elements
     *
     * @param accessionedModels
     * @return
     */
    public SaveResponse<ACCESSION> doSaveAccessions(
            List<ModelWrapper<MODEL, HASH, ACCESSION>> accessionedModels) {
        ...
        while (!partitions.isEmpty()) {
            ...
            final List<ModelWrapper<MODEL, HASH, ACCESSION>> partitionToSave = accessionedModels
                    .subList(partition.start, partition.end);
            try {
                dbService.insert(partitionToSave);
